...
def clean_json(input_file_path, dictionary_file_path, output_file_path):
    ...
    for key, value in data.items():
        ...
        # 检查键是否在字典文件中或符合保留条件
        if key not in removal_dict and not (number_pattern.match(key) or english_pattern.match(key) or
                                            numbers_comma_pattern.match(key) or letters_punctuation_pattern.match(
                    key) or
                                            ...
                                            operators_letters_punctuation_pattern.match(
                                                key) or punctuation_only_pattern.match(key) or
                                            slash_letters_numbers_punctuation_pattern.match(
                                                key) or letters_space_pattern.match(key) or
                                            letters_space_operators_pattern.match(
                                                key) or letters_space_punctuation_pattern.match(key)):
            new_data[key] = value
